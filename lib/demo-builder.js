...
function filterItemsFor(assets, param) {
  return _
    .chain(assets)
    .map(param)
    .without(undefined)
...
function filtersFor(assets, filters) {
  return filters.map(function(filter) {
    return {
      name: filter.name,
      param: filter.param,
      items: filterItemsFor(assets, filter.param)
    };
  });
}
...
DemoBuilder.prototype.getDemoModel = function() {
  ...
  return {
    sortBy: [
      { name: 'Asset name', param: 'name' },
      { name: 'Base size', param: 'baseSize' }
    ],

    arrangeBy: [
      { name: 'Directory', param: 'fileDir' },
      { name: 'Base size', param: 'baseSize' }
    ],

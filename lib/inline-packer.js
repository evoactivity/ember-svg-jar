...
class InlinePacker extends CachingWriter {
  ...
  build() {
    ...
    this.listFiles()
      ...
      .forEach((filePath) => {
        let relativePath = utils.relativePathFor(filePath, inputPath);
        let modulePath = path.join(outputPath, `${assetIdFor(relativePath)}.js`);
        let svgData = extractSvgData(filePath);
        let moduleContents = `export default ${JSON.stringify(svgData)}`;
        utils.saveToFile(modulePath, moduleContents);
      });
  }
}

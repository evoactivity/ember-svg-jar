...
const templateOnlyComponentFn = (layoutName) => `
import Component from '@ember/component';
import layout from 'ember-svg-jar/components/${layoutName}';
...
class HbsPlugin extends Plugin {
  ...
  build() {
    ...
    components.forEach((component) => {
      const chunkPathHbs = path.join(
        this.outputPath,
        'components',
        `${component.name.dashCase}.hbs`
      );
      ensureDirectoryExistence(chunkPathHbs);
      fs.writeFileSync(chunkPathHbs, component.chunk.code);

      // if (!hasBackingJS(component.chunks)) {
        const chunkPathJs = path.join(
          this.outputPath,
          'components',
          `${component.name.dashCase}.js`
        );
        ensureDirectoryExistence(chunkPathJs);
        fs.writeFileSync(chunkPathJs, templateOnlyComponent);
        console.log(chunkPathJs)
      // }
    });
  }
}
...
module.exports = {
  ...
  treeForApp(tree) {
    ...
    const hbsTree = new HbsPlugin(this.svgsFor('inline'));
    trees.push(hbsTree);

...
function mergeTreesIfNeeded(trees, options) {
  const mergedOptions = _.assign({ overwrite: true }, options);
...
module.exports = {
  ...
  originalSvgsFor(strategy) {
    const sourceDirs = this.sourceDirsFor(strategy);
  ...
  svgsFor(strategy) {
    const originalSvgs = this.originalSvgsFor(strategy);
  ...
  viewerSvgsFor(strategy) {
    const originalSvgs = this.originalSvgsFor(strategy);
    const nodes = [originalSvgs];

    if (this.hasOptimizerFor(strategy)) {
      const optimizedSvgs = this.optimizedSvgsFor(strategy, originalSvgs);
  ...
  getViewerTree() {
    ...
    const viewerBuilderNodes = this.svgJarOptions.strategy.map((strategy) => (

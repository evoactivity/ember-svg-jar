...
function svgSizeFor(svgAttrs) {
  let viewBoxData = (svgAttrs.viewBox || '').split(/\s+/);
  let vbWidth = viewBoxData[2];
  let vgHeight = viewBoxData[3];
...
const svgToAsset = _.curry(
  (relativeToId, options) => {
    let relativePath = options[0];
    let originalSvg = options[1];
    let optimizedSvg = options[2];
...
const assetToViewerItem = _.curry((copypastaGen, strategy, asset) => {
  ...
  let optimizedSvg = asset.optimizedSvg;
...
class ViewerAssetsBuilder extends CachingWriter {
  ...
  build() {
    ...
    let hasOptimizer = this.options.hasOptimizer;
    ...
    */
    fp.pipe(
      ...
      fp.tap((assets) => ui && validateAssets(assets, validations, strategy, ui)),

...
function toViewerAsset(relativePath, svgContent, opts) {
...
class ViewerAssetsBuilder extends CachingWriter {
  ...
  build() {
    let { makeAssetID, validationConfig, strategy } = this.options;
    ...
    let viewerAssets = filePaths.map(({ filePath, relativePath }) => (
      toViewerAsset(relativePath, readFile(filePath), this.options)
    ));
